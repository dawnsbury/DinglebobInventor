...
using AdventuresOnThisPoint.PirateAttack;

namespace AdventuresOnThisPoint
{
    public class ModMain
    {
        ...
        public static QEffect RechargingAction(string actionName, Dice rechargeAfterHowManyTurns = Dice.D4)
        {
            int value = R.Next(2, (int)(rechargeAfterHowManyTurns + 2));
            return new QEffect("Recharging " + actionName, "This creature can't use " + actionName + " until the value counts down to zero.", ExpirationCondition.CountsDownAtEndOfYourTurn, null, IllustrationName.Recharging)
            {
                Id = QEffectId.Recharging,
                CountsAsADebuff = true,
                PreventTakingAction = (CombatAction ca) => ca.Name != null && ca.Name.Contains(actionName) ? "This ability is recharging." : null,
                Value = value
            };
        }
